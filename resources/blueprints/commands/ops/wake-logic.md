...
6.  Detect Onboarding: 
    - Check if `{{WORKSPACE}}PROJECT_CONTEXT.md` contains "{{PROJECT_NAME}}" (Template state).
    - Check if `{{WORKSPACE}}PROJECT_CONTEXT.md` has "Tour Completed: false".
...
1. After user replies with project details, you MUST immediately:
   - Use "replace" or "write_file" to update "{{WORKSPACE}}PROJECT_CONTEXT.md".
...
## 7. ERROR HANDLING
- If `{{WORKSPACE}}PROJECT_CONTEXT.md` is missing or empty: Suggest running `/kamiflow:ops:save-context`.
- If `{{WORKSPACE}}ROADMAP.md` is missing: Suggest running `/kamiflow:ops:roadmap`.
