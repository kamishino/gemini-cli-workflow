...
async function createIdea(title, content, aiSlug, type = "draft") {
  try {
    ...
    let fileName = isDiscovery
      ? `${id}_${slug}_ai-discovery.md`
      : `${id}-${slug}.md`;
    ...
    while (await fs.pathExists(targetPath)) {
      ...
      fileName = isDiscovery
        ? `${newId}_${slug}_ai-discovery.md`
        : `${newId}-${slug}.md`;
...
async function analyzeIdea(filePath, scores) {
  ...
  value: ${scores.value || 0}`;
      ...
      console.log(
        chalk.green(`✓ Scores updated: Feasibility=${scores.feasibility}`),
      );
    } else {
      ...
      const { action } = await inquirer.prompt([
        {
          ...
          choices: [
            ...
            {
              name: "🚀 Force Promote (I know what I am doing)",
              value: "FORCE",
            },
          ],
        },
