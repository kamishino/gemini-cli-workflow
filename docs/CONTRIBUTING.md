...
```
gemini-cli-workflow/
├── packages/kamiflow-cli/                    # Main CLI package
...
1. **Location:** Place tests in `packages/kamiflow-cli/tests/` matching source structure

   ```
   packages/kamiflow-cli/logic/config-manager.js
   packages/kamiflow-cli/tests/logic/config-manager.test.js
   ```
...
1. **Update locale files:**
   - `packages/kamiflow-cli/locales/en.json`
   - `packages/kamiflow-cli/locales/vi.json`
