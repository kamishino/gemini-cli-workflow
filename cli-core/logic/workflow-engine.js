...
class WorkflowEngine {
  ...
  async saveArtifact(params) {
    ...
    if (phase !== "IDEA" && score < 8.0) {
      throw new Error(
        `[Hard Gate] Confidence Score (${score}) is below the required 8.0 threshold. Please refine the IDEA phase.`,
      );
    }
    ...
    const currentMetadata = await this.getTaskState(taskId).then(
      (s) => s?.metadata || {},
    );
    const updatedMetadata = { ...currentMetadata, ...metadata };
    updatedMetadata.artifacts = updatedMetadata.artifacts || {};
    updatedMetadata.artifacts[phase.toLowerCase()] = path.relative(
      this.projectRoot,
      filePath,
    );
