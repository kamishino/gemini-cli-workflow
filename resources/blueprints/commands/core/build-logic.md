...
### 🔒 LOGIC DRIFT CHECK (MANDATORY)

During the Lock 3 Reconnaissance, you MUST verify if the code facts match the SPEC:
1. **Compare Facts:** Does the existing code support the architecture defined in S2-SPEC?
2. **Detect Drift:** If the code reality contradicts the Spec (e.g., missing dependencies, different function signatures, or architectural blockers):
   - **ACTION:** Lower the confidence level.
   - **ACTION:** STOP and explicitly suggest running `/kamiflow:dev:revise` to re-align the plan with reality.
   - **RULE:** NEVER proceed with a plan that ignores code facts.
...
- [ ] **Task 4.0: Execute Validation Loop** (MANDATORY)
  - **Protocol:** Follow `{{KAMI_RULES_GEMINI}}flow-validation-core.md`
...
- [ ] **Task 4.6: Stage Changes (Unified Commit Prep)**
  - **Action:** `git add .`
