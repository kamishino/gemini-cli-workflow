...
class PluginManager {
  ...
  async loadInstalledPlugins() {
    ...
    if (await fs.pathExists(pluginsConfigPath)) {
      ...
      for (const [name, installConfig] of Object.entries(
        config.plugins || {},
      )) {
        ...
        if (validation.valid) {
          ...
          logger.warn(
            `Invalid plugin config for ${name}: ${validation.errors[0].message}`,
          );
        }
      }
    }
  ...
  async install(identifier, options = {}) {
    ...
    if (!validation.valid) {
      throw new Error(
        `Invalid plugin manifest: ${validation.errors[0].message}`,
      );
    }
    ...
    if (manifest.engines && manifest.engines.kamiflow) {
      if (
        !checkVersionCompatibility(manifest.engines.kamiflow, kamiflowVersion)
      ) {
        throw new Error(
          `Plugin requires KamiFlow ${manifest.engines.kamiflow}, but ${kamiflowVersion} is installed`,
        );
      }
    }
  ...
  async installFromGit(gitUrl, version = "main") {
    ...
    try {
      ...
      await execAsync(
        `git clone --depth 1 --branch ${version} ${gitUrl} ${tempDir}`,
      );
  ...
  async installFromNpm(packageName, version = "latest") {
    ...
    // Install package
    await execAsync(
      `npm install ${packageName}@${version} --prefix ${targetPath} --no-save`,
    );
  ...
  async installFromRegistry(pluginName, version = "latest") {
    ...
    throw new Error(
      "KamiFlow registry not yet available. Use --source git or --source local",
    );
  }
