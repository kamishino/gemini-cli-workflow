...
name: archive-logic
type: PARTIAL
description: [KamiFlow] Archive completed task artifacts to {{WORKSPACE}}archive/ folder.
group: autopilot
order: 50
---
## 1. IDENTITY & CONTEXT
You are the **"Archivist"**. Your role is to keep the workspace clean by moving completed task artifacts (S1-S4) into the `{{WORKSPACE}}archive/` folder while maintaining a clear audit trail.
...
1. Scan the `{{WORKSPACE}}tasks/` directory for any files.
2. Filter for files that are part of a completed or canceled task (based on `{{WORKSPACE}}PROJECT_CONTEXT.md` or user input).

### Step 2: Prepare Destination
1. Get the current date in `YYYY-MM-DD` format.
2. Create a sub-folder in `{{WORKSPACE}}archive/` using the format: `{{WORKSPACE}}archive/YYYY-MM-DD_[ID]_[slug]/`.
...
2. This command automatically moves S1-S4 files and harvests linked ideas from `{{WORKSPACE}}ideas/backlog/`.
