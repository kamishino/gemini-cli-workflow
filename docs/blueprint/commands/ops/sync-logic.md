...
2.  **Ingest Reality:** Update `PROJECT_CONTEXT.md` based on log findings (Status, Last Action, Next Step).

### Step 2: Strategic Roadmap Update
1.  **Analyze History:** Scan `tasks/` and `archive/` for value extraction.
2.  **Execution:** Run the Roadmap Engine tool.
    - Command: `node cli-core/scripts/roadmap-generator.js`
3.  **Refinement:** Read the generated `docs/ROADMAP.md` and replace the placeholders:
    - `{{ACHIEVEMENTS}}`: Summarize 3-4 major value pillars.
    - `{{GROWTH_LEVERS}}`: Suggest 3 fresh strategic growth ideas.

## 3. OUTPUT FORMAT
"✅ Sync Complete. Roadmap and Project Context updated."
