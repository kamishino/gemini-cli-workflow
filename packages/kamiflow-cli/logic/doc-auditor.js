...
function checkVersionDrift(rootDir) {
  const issues = [];
  try {
    const packageJsonPath = path.join(rootDir, "packages/kamiflow-cli/package.json");
...
async function applyFixes(issues, options, rootDir) {
  ...
  if (issues.some((i) => i.type === "VERSION_MISMATCH")) {
    ...
    try {
      execSync("node packages/kamiflow-cli/scripts/sync-version.js", { stdio: "inherit" });
      execSync("node packages/kamiflow-cli/scripts/sync-docs.js", { stdio: "inherit" });
