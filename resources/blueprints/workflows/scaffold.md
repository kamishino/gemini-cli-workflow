...
- You need a new agent, workflow, or rule quickly with consistent structure.
- You want the CLI to generate file scaffolding before refining content.
- You need to standardize prompt/rule quality across a team.
...
4. **Intelligent Content Injection**
   ...
   - Using your intelligence as an AI assistant, craft the prompt/instructions/rules based on the user's description and project context.
   - If it's an **Agent**: fill in `triggers` and `owns` in YAML frontmatter. Fill `# Identity`, `# Rules`, and `# Behavior` with actionable instructions.
...
| :---------- | :------------------------------------------------ |
| `/research` | Explore patterns before scaffolding complex items |
| `/develop`  | Continue implementation after scaffold generation |
| `/review`   | Validate generated content quality before merge   |
